...
def main():
    ...
    optional.add_argument(
        "-l", "--labels", metavar="FILE", help="path to label names (e.g. coco.names)"
    )
    optional.add_argument(
        "-o", "--output-uri", metavar="URI", help="URI to output video file"
    )
    optional.add_argument(
        "-t",
        "--txt",
    ...
    optional.add_argument(
        "-m", "--mot", action="store_true", help="run multiple object tracker"
    )
    optional.add_argument(
        "-s", "--show", action="store_true", help="show visualizations"
    )
    group.add_argument(
    ...
    with open(args.config) as cfg_file:
        config = json.load(
            cfg_file, cls=ConfigDecoder, object_hook=lambda d: SimpleNamespace(**d)
        )
    ...
    stream = fastmot.VideoIO(
        config.resize_to, args.input_uri, args.output_uri, **vars(config.stream_cfg)
    )
